...
function classIFrame()
{
	...
	{			
		...
		this.N.push($('<div class="title-nav-btn i-br tooltip" onClick="parent.Frame.IFrame.open(\'research\');" data-tooltip-content="'+LNG['lm_research']+'"></div>'));
		this.N.push($('<div class="title-nav-btn i-bs tooltip" onClick="parent.Frame.IFrame.open(\'shipyard\');" data-tooltip-content="'+LNG['lm_shipshard']+'"></div>'));
		this.N.push($('<div class="title-nav-btn i-bb tooltip" onClick="parent.Frame.IFrame.open(\'buildings\');" data-tooltip-content="'+LNG['lm_buildings']+'"></div>'));			
...
{	
	...
	{	
		...
		if(data.F !== undefined)
		{
			//-- Ð¡Ð¸Ð³Ð½Ð°Ð»
			if(data.F.a > 0)
				this.D['s_a'].addClass('menu-s-i-a');
			else
				this.D['s_a'].removeClass('menu-s-i-a');
				
			if(data.F.d > 0)
				this.D['s_d'].addClass('menu-s-i-a');
			else
				this.D['s_d'].removeClass('menu-s-i-a');
			
			//-- Ð¡Ð¸Ñ€ÐµÐ½Ð°
			if(data.F.a > Frame.fAttack)
				this.A[0].play();
			
			if(data.F.d > Frame.fDestr)
				this.A[0].play();
...
function classResNav()
{
	...
	this.p = {901:[0,0,0], 902:[0,0,0], 903:[0,0,0], 911:[0,0,0], 921:[0,0,0], 931:[0,0,0]};
	/*
	 * 0 - ÐšÐ¾Ð»0-Ð²Ð¾
	 * 1 - ÐœÐ°ÐºÑÐ¸Ð¼ÑƒÐ¼
	...
	*/
	...
	{	
		...
		//-- ÐžÐ¾Ð±Ñ€Ð°Ð¶Ð°ÐµÐ¼ Ð·Ð°Ð¿Ð¾Ð»Ð½ÐµÐ½Ð¾ÑÑ‚ÑŒ
		if(this.p[id][1] != 0)
		{
...
{	
	...
	{	
		...
		//-- Build
		this.QB['block']	= $('<div class="queue-block" onClick="Frame.IFrame.open(\'buildings\');"><div class="queue-img"></div><div class="queue-level"></div></div>');
		...
		//-- shipyard
		this.QS['block']	= $('<div class="queue-block" onClick="Frame.IFrame.open(\'shipyard\');"><div class="queue-img"></div><div class="queue-count"></div></div>');
		...
		//-- research
		this.QR['block']	= $('<div class="queue-block" onClick="Frame.IFrame.open(\'research\');"><div class="queue-img"></div><div class="queue-level"></div></div>');
	...
	{	
		...
		this.setBb(this.QB['b_b_b'], Queue[0]['element'], Queue[0]['level']);		
		...
		for(i = 1; i <= maxQ; i++) 
		{
			...
			this.setBb(this.QB['b_'+i], Queue[i]['element'], Queue[i]['level']);
		}
	...
	{
		b.find('.queue-img:first').attr('class', 'queue-img i-add');
		b.find('.queue-level:first').text('');			
		b.show();
	}
	//-- -- Build Ð£ÑÑ‚Ð°Ð½Ð¾Ð²ÐºÐ° Ð—Ð°Ð½ÑÑ‚Ð¾Ð³Ð¾ Ð±Ð»Ð¾ÐºÐ°
	this.setBb = function(b,i,c) 
	{
		b.find('.queue-img:first').attr('class', 'queue-img i-u' + i);
	...
	this.setQS = function(data) 
	{
		...
		this.setSb(this.QS['b_b_b'], Queue[0][3], Queue[0][1]);
		...
		for(i = 1; i <= maxQ; i++) 
		{
			...
			this.setSb(this.QS['b_'+i], Queue[i][3], Queue[i][1]);
		}
	...
	{		
		...
		this.QR['b_b_c'].show();
		this.QR['b_b_t'].show();
		this.setRb(this.QR['b_b_b'], Queue['id'], Queue['level']);
	...
	this.setRb = function(b,i,c) 
	{
		b.find('.queue-img:first').attr('class', 'queue-img i-u' + i + 'b');
		b.find('.queue-level:first').text(c);	
		b.show();
	}
	//-- -- Research Timer
...
function classChat()
{
	...
	this.construct = function()
	{			
		this.B	= $('<div id="chat"></div>');
		this.P	= $('<div id="chat-preloader"><div class="fancybox-loading"></div></div>');
		this.C	= $('<div id="chat-close">&times;</div>');
		this.I	= $('<iframe name="iframe-cat" id="iframe-cat" frameborder="0" width="100%" height="100%" scrolling="auto" src="//woa.xterium.space/game.php?page=chat&mini_chat=1&default_chat=1"></iframe>');
					
		this.C.click(function(){Frame.Chat.close();}); //https://woa.xterium.space/game.php?page=chat&mini_chat=1&default_chat=1
