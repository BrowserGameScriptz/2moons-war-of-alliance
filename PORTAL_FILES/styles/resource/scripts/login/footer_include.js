...
function loadEvents()
{
  jQuery.post("./loadEvents.php", {'ajax': 1}, function(data)
    {
      fadeInDiv();
    }, "json"
  );
}
//jusque 158px width on peu aller top 0 et botom height 250
function fadeInDiv(){
    var divs = $('.fadeIn');
    var divsize = ((Math.random()*100)).toFixed();
    var posx = (Math.random() * (1120 - divsize)).toFixed();
    var posy =  (Math.floor(Math.random() * (200 - 150 + 1)) + 150).toFixed();
    var size = 14;
    var elem = divs.eq(Math.floor(Math.random()*6));
    
	elem.fadeOut(Math.floor(2500),fadeInDiv); 
	
    if (!elem.is(':visible')){
        elem.fadeIn(Math.floor(Math.random()*7000),fadeInDiv);
		//elem.html('This is element at pos ' +posx+'x'+posy+'px');
        elem.css({
            'position':'absolute',
            'left':posx+'px',
            'top':posy+'px',
			'background-color':'#222',
			'background-image':'-moz-linear-gradient(#3a3a3a 0,#222 100%)',
			'background-image':'-webkit-gradient(linear,color-stop(0,#3a3a3a),color-stop(1,#222))',
			'background-image':'-webkit-linear-gradient(#3a3a3a 0,#222 100%)',
			'background-image':'-o-linear-gradient(#3a3a3a 0,#222 100%)',
			'background-image':'-ms-linear-gradient(#3a3a3a 0,#222 100%)',
			'background-image':'linear-gradient(#3a3a3a 0,#222 100%);-moz-box-shadow:1px 1px 4px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.1)',
			...
			'line-height':'16px;color:#ccc;padding:5px 10px',
			'border-radius':'5px',
            'font-size': size+'px'
        });
    } else {
