...
require 'includes/classes/DatabaseAjax.class.php';
require 'includes/classes/ConfigAjax.class.php';
...
require 'includes/classes/SessionAjax.class.php';
...
if(defined('DATABASE_VERSION') && DATABASE_VERSION === 'OLD')
{
	...
	$dbTableNames	= DatabaseAjax::get()->getDbTableNames();
...
if (MODE === 'INGAME' || MODE === 'ADMIN' || MODE === 'CRON')
{
	...
	$USETT	= $db->selectSingle($sql, array(
		':userId'	=> $session->userId
	));
...
elseif(MODE === 'LOGIN')
{
	$session = sessionajax::load();
	if($session->isValidSession())
	{
		$sql	= "SELECT * FROM %%USERS%% WHERE id = :userId;";
		$AccountInfo	= DatabaseAjax::get()->selectSingle($sql, array(
			':userId'	=> $session->userId
		)); 
		$LNG	= new Language($AccountInfo['lang']);
