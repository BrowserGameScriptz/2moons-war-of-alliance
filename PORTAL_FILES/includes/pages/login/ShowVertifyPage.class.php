...
class ShowVertifyPage extends AbstractLoginPage
{
	...
	private function _activeUser()
	{
		...
		if(empty($userData))
		{
			$this->printMessage($LNG['vertifyNoUserFound'], true, array('index.php?page=register', 2));
		}
		...
		list($userID) = PlayerUtil::createPlayer($userData['universe'], $userData['userName'], $userData['password'], $userData['email'], $userData['language'], $userData['displayname'], $userData['secretQuestion'], $userData['secretAnswer'], $userData['birthday'], $userData['referralID']);
		...
		{
			$sql = "UPDATE %%USERS%% SET `ref_id` = :referralId, `ref_bonus` = 1 WHERE `id` = :userID;";
			$db->update($sql, array(
	...
	function show()
	{
		...
		$this->printMessage('Registration succesful: you can now login to join the game.', true, array('index.php', 3));
	}
