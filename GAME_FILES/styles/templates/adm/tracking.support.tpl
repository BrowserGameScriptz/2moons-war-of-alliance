						...
						<table id="subdomaintbl" class="sortable table table-striped">
							...
							<tbody>
								{foreach $trackLog as $logId => $LogRow}
									<tr>
										<td>{$logId}</td>
										<td>{$LogRow.subject}</td>
										<td>{$LogRow.answer}</td>
										<td>{$LogRow.time}</td>
										<td>{if $LogRow.status == 0}<span style="color:green">{$LNG.ti_status_open}</span>{elseif $LogRow.status == 1}<span style="color:orange">{$LNG.ti_status_answer}</span>{else}<span style="color:red">{$LNG.ti_status_closed}</span>{/if}</td>
									</tr>
								{/foreach}
							</tbody>
							<tfoot></tfoot>
