...
function setTooltip(t)
{
	t.find(".tooltip").on({
		...
		{
			tooltip.html($(this).data('tooltip-content'));
			if(tooltip.html() == '')
				return;
...
function setTooltipCoord(t, e, c)
{
	...
	var offset		= t.getBoundingClientRect();
...
function NotifyBox(text, time)
{
	...
	if(tooltime !== 0)
		window.clearInterval(tooltime);
		
	tooltime = window.setTimeout(function(){tooltip.fadeOut(1000); tooltime = -1;}, time);
}
