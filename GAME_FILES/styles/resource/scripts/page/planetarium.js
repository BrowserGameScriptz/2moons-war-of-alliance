function checkrename()
{
	...
	$.getJSON('game.php?page=planetarium&mode=rename&name='+$('#name').val(), function(response){
		parent.NotifyBox(response.message);
		if(!response.error) {
			location.reload();
		}
	});	
...
{		
	$.getJSON('game.php?page=planetarium&mode=GenerateName', function(response){
...
function checkcancel()
{
	...
	$.getJSON('game.php?page=planetarium&mode=delete&name='+$('#del_name').val(), function(response){
		parent.NotifyBox(response.message);
		if(response.ok){
