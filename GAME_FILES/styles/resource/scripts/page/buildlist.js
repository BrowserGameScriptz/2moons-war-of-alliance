...
function Buildlist() {
	...
	if (rest <= 0) {
		...
		window.setTimeout(function() {
			...
			IPost("game.php?page=buildings", {}, [interval], []);
		}, 1000);
		return true;
	}
	...
	interval = window.setTimeout(Buildlist, 1000);
}

function PostBuild(id, cmd, listid, level) 
{	
	IPost("game.php?page=buildings", {'cmd': cmd, 'building': id, 'listid': listid, 'level': level}, [interval], []);
	/*
	window.clearInterval(interval);
