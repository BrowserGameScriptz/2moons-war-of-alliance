...
class ResourceUpdate
{
	...
	public function CreateHash() {
		...
		foreach($reslist['prod'] as $ID) {
			...
			if(isset($this->PLANET[$resource[$ID]])){
				$Hash[]	= $this->PLANET[$resource[$ID]];
				$Hash[]	= $this->PLANET[$resource[$ID].'_porcent'];
			}elseif(isset($this->USER[$resource[$ID]])){
	...
	private function ExecCalc()
	{
		...
		$ResearchTheoretical	= $this->ProductionTime * (($this->config->research_basic_income * $this->config->resource_multiplier) + $this->USER['research_perhour']) / 3600;
		if ($ResearchTheoretical < 0)
		{
			$this->USER['research']    = max($this->USER['research'] + $ResearchTheoretical, 0);
		} 
		elseif($this->USER['research'] <= $MaxResearchStorage) 
		{
			$this->USER['research']    = min($this->USER['research'] + $ResearchTheoretical, $MaxResearchStorage);
		}
		...
		$this->USER['research']		= max($this->USER['research'], 0);
	}
	...
	public function ReBuildCache()
	{
		...
		if ($this->PLANET['planet_type'] == 3)
		{
			...
			$this->config->research_basic_income 	= 0;
		}
		...
		{	
			if(isset($this->PLANET[$resource[$ProdID]]))
				$WichTableChoose = $this->PLANET;
			elseif(isset($this->USER[$resource[$ProdID]]))
				$WichTableChoose = $this->USER;
			...
			foreach($ressIDs as $ID) 
			{
				...
				if($Production > 0) {					
					...
					if(in_array($ID, $reslist['resstype'][1]) && $WichTableChoose[$resource[$ID]] == 0) {
						 continue;
					}
		...
		$this->USER['research_max']			= 100000000;

		$this->PLANET['energy']				= round($temp[911]['plus'] * $this->config->energySpeed * (1 + $this->USER['factor']['Energy'] + 0.05 * $this->USER[$resource[113]]));
		...
		if($this->PLANET['energy_used'] == 0) {
			...
			$this->USER['research_perhour']		= 0;
		} else {
			$prodLevel	= min(1, $this->PLANET['energy'] / abs($this->PLANET['energy_used']));
			
			$this->PLANET['metal_perhour']		= ($temp[901]['plus'] * (1 + $this->USER['factor']['Resource'] + 0.05 * $this->USER[$resource[102]]) * $prodLevel + $temp[901]['minus']) * $this->config->resource_multiplier;
			$this->PLANET['crystal_perhour']	= ($temp[902]['plus'] * (1 + $this->USER['factor']['Resource'] + 0.05 * $this->USER[$resource[102]]) * $prodLevel + $temp[902]['minus']) * $this->config->resource_multiplier;
			$this->PLANET['deuterium_perhour']	= ($temp[903]['plus'] * (1 + $this->USER['factor']['Resource'] + 0.05 * $this->USER[$resource[102]]) * $prodLevel + $temp[903]['minus']) * $this->config->resource_multiplier;
			$this->USER['research_perhour']		= ($temp[941]['plus'] * (1 + $this->USER['factor']['Resource']) * $prodLevel + $temp[941]['minus']);
		}
	}
	...
	public function SetNextQueueElementOnTop()
	{
		...
		while ($Loop === true)
		{
			...
			if($HaveResources === true) {
				...
				if(isset($costResources[941])) { $this->USER[$resource[941]]	-= $costResources[941]; }
	...
	public function SavePlanetToDB($USER = NULL, $PLANET = NULL)
	{
		...
		$params	= array(
			...
			':research_max'			=> $USER['research_max'],
			':research_perhour'		=> $USER['research_perhour']
		);
		...
		u.research			= :research,
		u.research_max		= :research_max,
		u.research_perhour	= :research_perhour
		'.implode("\n", $buildQueries).'
