...
class PlayerUtil
{
	...
	static public function createPlayer($universe, $userName, $userPassword, $userMail, $userLanguage = NULL, $displayname, $secretQuestion, $secretAnswer, $birthday, $securityEncodage, $securityCode, $validationID, $galaxy = NULL, $system = NULL, $position = NULL, $name = NULL, $authlevel = 0, $userIpAddress = NULL)
	{
		$config	= Config::get($universe);
	...
	static public function createPlanet($galaxy, $system, $position, $universe, $userId, $name = NULL, $isHome = false, $authlevel = 0)
	{
		...
		$bonusOne	   = mt_rand(80,200)/10;
		$bonusTwo      = mt_rand(80,200)/10;
		if(mt_rand(0,100) >= 75)
			$bonusTwo      = mt_rand(80,480)/10;
		
		$isPositiveCon = mt_rand(0,100);
		$isPositiveNum = mt_rand(0,100);

		$constructionBonus  = $isPositiveCon >= 40 ? $bonusOne : $bonusOne*-1;
		$deuteriumBonus 	= $isPositiveNum >= 50 ? $bonusTwo : $bonusTwo*-1;
		$solarBonus 		= $isPositiveNum >= 50 ? $bonusTwo*-1 : $bonusTwo;
	...
	static public function maxPlanetCount($USER)
	{
		...
		return (int) 1 + $USER[$resource[103]];
	}
