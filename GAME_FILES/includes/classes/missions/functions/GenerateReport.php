...
function GenerateReport($combatResult, $reportInfo)
{
	...
	$DATA['debrisP']= round($reportInfo['debris'][901]/40000+ $reportInfo['debris'][902]/20000);
	$DATA['profitA']= round($reportInfo['debris'][901]/40000+ $reportInfo['debris'][902]/20000)-$combatResult['unitLost']['attacker'];
	$DATA['profitD']= round($reportInfo['debris'][901]/40000+ $reportInfo['debris'][902]/20000)-$combatResult['unitLost']['defender'];
	...
	foreach($combatResult['rw'] as $Round => $RoundInfo)
	{
		...
		{	
			...
			foreach($player['unit'] as $ShipID => $Amount)
			{
				...
				$farkA      = abs($Amount-$combatResult['rw'][$round_no]['attackers'][$FleetID]['unit'][$ShipID]);
		...
		{	
			...
			foreach($player['unit'] as $ShipID => $Amount)
			{
				...
				$farkD   	= abs($Amount-$combatResult['rw'][$round_no]['attackers'][$FleetID]['unit'][$ShipID]);
