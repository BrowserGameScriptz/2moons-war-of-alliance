...
function ShowTranslatePage()
{
	global $LNG, $USER, $LNGBIS;
		...
		*/
		...
		if ($lang != 'en' && file_exists($translationfile)) {
			...
			/* $languageListShow .= '<h2><a name="compare"></a>Direct comparision between english and '.htmlspecialchars($lang).'</h2>';
			$languageListShow .= '<table>
				<colgroup></colgroup>
				<thead><tr>
			...
			$i = 0;
				foreach ($orig_language as $key => $val) {
					if (!isset($translation[$key])) {
					  $languageListShow .= '<tr><th>'.$key.'</th><td>'.htmlspecialchars($val).'</td><td></td></tr>';
					}else{
					  $languageListShow .= '
			<tr>
			...
			if ($handle = opendir('lang')) {
				...
				foreach($langfiles as $lang){
					...
					if($lang!='en.php'){
						$languageListShow .=  '<tr><td>'.substr($lang,0,-4).'</td><td>Production File</td><td><a href="admin.php?page=translate&lang='.substr($lang,0,-4).'" class="progress_bar_container">
		<span style="width:'.$progress.'%" class="progress_bar"></span></a></td><td><a href="admin.php?page=langedit&lang='.substr($lang,0,-4).'">Translate</a></td></tr>';
		...
		<span style="width:100%" class="progress_bar"></span></a></td><td>-</td></tr>';
					}
				}
				foreach($workfiles as $workfile){
					$languageListShow .=  '<tr><td>'.substr($workfile,1,-9).'</td><td>Development File - The file change have not been confirmed yet.</td><td><a href="admin.php?page=translate&lang='.substr($workfile,1,-9).'" class="progress_bar_container">
		<span style="width:'.$progress.'%" class="progress_bar"></span></a></td><td><a href="admin.php?page=langedit&lang='.substr($workfile,1,-9).'">Cofmrim the changes of  '.substr($workfile,1,-9).'</a></td></tr>';
				}
				closedir($handle);
	...
	$template->show('translate.tpl');
}

function ShowNewlanguagePage()
{
	global $LNG, $USER;
	
	if (!empty($_POST))
	{
		$langTag			= HTTP::_GP('langTag', '', true);
		
		if(!empty($langTag))
			header('Location: ?page=langedit&lang='.$langTag);
	...
	$template->show('newlanguage.tpl');
}

function ShowLangeditPage()
{
	global $LNG, $USER, $LNGBIS;
		...
		if($lang == 'en')
			header('Location: admin.php?page=translate');
		...
		if($fail) {
			die($fail."<b>Usage:</b> <a href='../index.php?page=langedit&lang='>&lt;lang code&gt;</a> where &lt;lang code&gt; should be replaced by your language, e.g. <b>de</b> for German.");
		}
		...
		// Path to this file
		$self = $_SERVER['SCRIPT_NAME']."?page=langedit&lang=".$lang;
		...
		$showBlock = "<form method=post action=?page=langedit&lang=".$lang."&begin=".$begin.">";
