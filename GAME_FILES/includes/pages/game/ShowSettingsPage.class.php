...
class ShowSettingsPage extends AbstractGamePage
{
	...
	public function show()
	{
		...
		if($USER['urlaubs_modus'] == 1)
		{
			$this->assign(array(
				...
				'vacationUntil1'		=> sprintf($LNG['settings_10'], _date($LNG['php_tdformat'], $USER['urlaubs_until'], $USER['timezone'])),
		...
		else
		{
			$this->assign(array(
				...
				'settingAlarm'		=> $USETT['settingAlarm'],	
				'autoHideFleet'		=> $USETT['autoHideFleet'],	
	...
	private function sendDefault()
	{
		...
		$vacation			= HTTP::_GP('vacation', 0);	
		$autoHideFleet		= HTTP::_GP('autointing', 0);	
		//$delete				= HTTP::_GP('delete', 0);
		...
		/* if (!empty($username) && $USER['username'] != $username)
		{
			...
			else
			{
				...
				if (!empty($Count)) {
					$this->printMessage($LNG['op_change_name_exist'], true, array('game.php?page=settings', 2));
				} else {
		...
		/* if (!empty($email) && $email != $USER['email'])
		{
			...
			{
				$this->printMessage($LNG['op_need_pass_mail'], true, array('game.php?page=settings', 2));
			}
			elseif(!ValidateAddress($email))
			{
				$this->printMessage($LNG['op_not_vaild_mail'], true, array('game.php?page=settings', 2));
			}
			else
			{
				$sql = "SELECT
				...
				if (!empty($Count)) {
					$this->printMessage($LNG['op_change_mail_exist'], true, array('game.php?page=settings', 2));
				} else {
		...
		$sql = "UPDATE %%USETT%% SET settingAlarm = :sirena, autoHideFleet = :autoHideFleet WHERE id = :userID";
		$db->update($sql, array(
			':sirena'			=> $sirena,
			':autoHideFleet'	=> $autoHideFleet,
			':userID'			=> $USER['id'],
		));
