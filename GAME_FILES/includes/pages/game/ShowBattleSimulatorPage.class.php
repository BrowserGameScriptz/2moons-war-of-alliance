...
class ShowBattleSimulatorPage extends AbstractGamePage
{
	...
	function send()
	{
        ...
        $this->sendJSON(array($reportID, "1"));
	}
	...
	function show()
	{
		...
		$ResearchIds	= array(109,111,110,171,172,173,141,142,143,120,121,122,199,155,144,174);
		$FleetsIds		= array(210,212,202,209,208,203,204,206,205,207,213,225,215,211,216,232,235,214,226,233,222,221,229,224,231,230);
		$DefenseIds		= array(407,408,409,402,405,406,410,414,403,404,423,413,418,416,434,412,419,417,415,435,420,421,422,443,);
		$fleetArray		= array();
		$DefenseArray	= array();
		
		foreach($ResearchIds as $Ids){
			$BattleArray[0][0][$Ids]	= $USER[$resource[$Ids]];
		}
		
		if(empty($_REQUEST['battleinput']))
		{
			foreach($FleetsIds as $ID)
			{
		...
		foreach($FleetsIds as $Ids){
			$fleetArray[$Ids]	= array(
				'points'	=> round(($pricelist[$Ids]['cost'][901]/40000 + $pricelist[$Ids]['cost'][902]/20000),3),
			);
		}
		
		foreach($DefenseIds as $Ids){
			$DefenseArray[$Ids]	= array(
				'points'	=> round(($pricelist[$Ids]['cost'][901]/40000 + $pricelist[$Ids]['cost'][902]/20000),3),
			);
		}
